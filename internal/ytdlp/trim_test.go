...
func TestTrimmerNormalizesTheFileWithStreamCopyAndReplacesIt(t *testing.T) {
	...
	err := (ytdlp.Trimmer{Binary: binary}).Trim(context.Background(), input, domain.TimeRange{Start: 5, End: 12})
	if err != nil {
		t.Fatalf("Trim: %v", err)
	}
	...
	assertContainsSequence(t, args, "-ss", "5", "-i", input, "-t", "7", "-map", "0", "-c", "copy", "-avoid_negative_ts", "make_zero")
